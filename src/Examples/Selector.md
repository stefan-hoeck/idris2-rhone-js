...
This imports all example applications and module `Examples.CSS`, where
...
interactive components in the DOM, and a way to register event
handlers.
...
taking a value of type `DomEnv ev` as input.
...
appearance of the examples web page.)
...
as having the follwing type:

```haskell
data MSF m i o = MSF (i -> m (o, MSF m i o))
```

This is an effectful function over a monad type `m` converting
...
But before doing so, every element that can fire an even is
...
IDs and add register the event handlers.

Afterwards, the monadic streaming function is created: We
use `arrM` to lift and effectful computation to the MSF context.
...
approach is that we can write a single (pure!) function that converts
the model to the view and never have to interact with
the real DOM explicitly. The downside is, that we loose some
control over which parts are of the web page are updated when,
...
functions. Whether this will result in a nice programming experience
